...
- (void)viewWillAppear:(BOOL)animated {
    [self sendRequestToGetTournamentList];
...
- (IBAction)handleDefaultButtonClick:(UIButton *)sender {
    ...
    if (flag == 0) {
        ...
        [self sendRequestToAddFavourite:[defaultTournamentDir valueForKey:@"id"]];
    ...
    if (sender.selected == YES) {
        if ([[defaultTournamentDir valueForKey:@"tournament_id"] integerValue] == sender.tag) {
            ...
            for (int i=0; i<_selectedArray.count; i++) {
                if ([[[_selectedArray objectAtIndex:i] valueForKey:@"tournament_id"] integerValue] == sender.tag) {
                    [self sendRequestToRemoveFavourite:[[_selectedArray objectAtIndex:i] valueForKey:@"tournament_id"]];
        ...
        for (int i =0; i<_tournamentlistArray.count; i++) {
            if ([[[_tournamentlistArray objectAtIndex:i] valueForKey:@"id"] integerValue] == sender.tag ) {
                ...
                [self sendRequestToAddFavourite:[[_tournamentlistArray objectAtIndex:i] valueForKey:@"id"]];
            }
        }
...
//    [cell.faverateBtn addGestureRecognizer:pgr];
    ...
    for (int i=0; i<_tournamentlistArray.count; i++) {
        ...
        */
        ...
        int tournament_id = [[defaultTournamentDir valueForKey:@"tournament_id"] intValue];
        int getID = [[[_tournamentlistArray objectAtIndex:indexPath.row] valueForKey:@"id"] intValue];
        if (tournament_id == getID) {
            cell.defaultBtn.selected = YES;
            break;
        } else {
            cell.defaultBtn.selected = NO;
...
- (void)sendRequestToGetTournamentList {
    if ([Utils isNetworkAvailable] == YES) {
        self.offlineView.hidden = TRUE;
        [SVProgressHUD show];
        ...
        NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request
                                                    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
                                          {
                                              if (error) {
                                                  ...
                                                  [self sendRequestToGetFavouriteTournament];
                                                  dispatch_async(dispatch_get_main_queue(), ^{
                                                      [self.tableView reloadData];
                                                  });
                                              }
...
- (void)sendRequestToGetFavouriteTournament {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        ...
        NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request
                                                    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error)
                                          {
                                              if (error) {
                                                  ...
                                                  NSDictionary *dicDefaultTournament = [[ApplicationData sharedInstance] getDefaultTournamentDic:responseDictionary[@"data"]];
...
- (void)sendRequestToSetDefaultTournament {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
...
- (void)sendRequestToAddFavourite:(NSString *)tournamentID {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSDictionary *userData = [defaults objectForKey:@"userData"];
    if ([Utils isNetworkAvailable] == YES) {
        [SVProgressHUD show];
...
- (void)sendRequestToRemoveFavourite:(NSString *)tournamentID {
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSDictionary *userData = [defaults objectForKey:@"userData"];
    if ([Utils isNetworkAvailable] == YES) {
        [SVProgressHUD show];
