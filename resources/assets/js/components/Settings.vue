		        	...
		        	<div class="col-9">
		           	...
		           	<div class="col-3">
						<form name="frmSettings" id="frmSettings" class="settings-form">
							<div class="form-group" :class="{'has-error': errors.has('tournament.screenRotateTime') }">
		              			<label>{{$lang.summary_setings_screen_rotate_time_message}}*</label>
		              			<input v-model="tournament.screenRotateTime" type="number" name="screenRotateTime" min="0" class="form-control" v-validate="'required'" :class="{'is-danger': errors.has('tournament.screenRotateTime') }">
		              			<i v-show="errors.has('tournament.screenRotateTime')" class="fas fa-warning"></i>
		          			</div>
  		...
  		saveSettings() {
    		...
    		this.tournament.tournamentId = this.$store.state.Tournament.tournamentId
	       	this.$validator.validateAll().then(
          		(response) => {
          			console.log('validateAll', response);
