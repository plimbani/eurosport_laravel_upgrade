									...
									<ul class="list-unstyled mb-4 round-details">
										<li v-if="group.type === 'round_robin'" v-for="(team, teamIndex) in group.teams">
											<div class="round-matches">
												<span class="w-130" v-if="roundIndex == 0">Team {{ teamIndex + 1 }}</span>
										...
										<li v-if="group.type === 'placing_match'" v-for="(team, teamIndex) in group.teams">
											<div v-if="teamIndex % 2 === 0">
												...
												<div v-if="roundIndex > 0" class="round-matches">
													<span :class="{'match-detail': groupPositionType(group.teams)}" v-html="getMatchDetail(team.position, team.position_type)"></span>
													<span>vs</span>
													<span v-if="roundIndex > 0" :class="{'match-detail': groupPositionType(group.teams)}" v-html="getMatchDetail(group.teams[teamIndex + 1].position, group.teams[teamIndex + 1].position_type)"></span>
												</div>
											</div>
										</li>
									</ul>
														...
														<li v-if="group.type === 'placing_match'" v-for="(team, teamIndex) in group.teams">
															<div v-if="teamIndex % 2 === 0" class="round-matches">
																...
																<div v-if="roundIndex > 0">
																	<span :class="{'match-detail': groupPositionType(group.teams)}" v-html="getMatchDetail(team.position, team.position_type)"></span>
																	<span>vs</span>
																	<span :class="{'match-detail': groupPositionType(group.teams)}" 
								...
								<div class="col-6">
									<ul class="list-unstyled mb-4 round-details">
										<li v-for="(placing, placingIndex) in templateFormDetail.stepthree.placings">
											...
											<!-- new code -->
											<div class="round-matches round-matches-placings">
												<span class="position-number">{{ getSuffixForPosition((placingIndex + 1)) }}</span>
												<span class="w-220" v-html="getMatchDetail(placing.position, placing.position_type)"></span>
											</div>
										</li>
									</ul>
								</div>
...
<script type="text/javascript">
	...
	export default {
        ...
        methods: {
            	...
            	if(teamPosition) {
			    	...
			    	if(groupData) {
				    	if(groupData.type === 'round_robin') {
				    		...
				    		return '<span class="position-number d-inline-block">' + this.getSuffixForPosition(position) + '</span><span class="pl-2">' + groupName +  '</span>';
				    	}
			...
			groupPositionType(teams) {
				return _.find(teams, function(o) { 
					if (o.position_type != 'placed') {
						return true;
					}
					return false;
				});
