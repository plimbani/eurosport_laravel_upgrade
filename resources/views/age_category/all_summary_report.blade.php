...
@foreach($data['age_group'] as $ageCategory)
  <center>
    <h3>{{ $ageCategory['ageCategoryName'] }}</h3>
  <center>
  @foreach($ageCategory['leagueTable'] as $league)
    <div class="row">
      <div class="col-sm-12">
  ...
  @foreach($ageCategory['resultGridTable'] as $competitionId => $resultGrid)
    @if($resultGrid['actual_competition_type'] == 'Round Robin')
      ...
      @endif
    @endif
    @if(isset($ageCategory['leagueTable'][$competitionId]))
      <h5>{{ $resultGrid['name'] }} standings</h5>
      @if(count($ageCategory['leagueTable'][$competitionId]['standings']) > 0)
        <table class="tblpage" border="1" cellpadding="1" cellspacing="0" width="100%" style="font-size: 70%">
          <thead>
            <tr>
              <th></th>
          ...
          <tbody>
            @foreach($ageCategory['leagueTable'][$competitionId]['standings'] as $match)
    ...
    @if(count($ageCategory['resultMatchesTable'][$competitionId]['results']) > 0)
      <table class="tblpage" border="1" cellpadding="1" cellspacing="0" width="100%" style="font-size: 70%">
        <thead>
          <th>Date and time</th>
          <th>Categories</th>
        ...
        <tbody>
          @foreach($ageCategory['resultMatchesTable'][$competitionId]['results'] as $match)
            <tr>
  ...
  @foreach($ageCategory['resultMatchesTableAfterFirstRound'] as $matches)
