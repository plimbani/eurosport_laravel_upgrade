              ...
              @foreach($group['groups']['match'] as $matchIndex=>$match)
                <div class="row-round">
                  @if(checkForMatchNumberOrRankingInPosition($templateData, 'placing_match', $match['match_number']))
                    <div class="bordered-box"><span class="font-weight-bold small">{{ checkForMatchNumberOrRankingInPosition($templateData, 'placing_match', $match['match_number']) }}</span></div>
                  @else
                    <div class="bordered-box"><span class="font-weight-bold small">Match {{ getMatchNumber($match['display_match_number']) }}</span></div>
                  @endif

                  @if(!checkIfWinnerLoserMatch($match['match_number']))
                    <div class="bordered-box"><span class="small">{{ getPlacingTeam($fixtures, $match, 'home', $groupName, $categoryAge) }}-{{ getPlacingTeam($fixtures, $match, 'away', $groupName, $categoryAge) }}</span></div>
                  @endif

                  @if(checkIfWinnerLoserMatch($match['match_number']))
                    <div class="bordered-box"><span class="small">{{ getPlacingWinnerLoserTeam($fixtures, $match, 'home', $groupName, $categoryAge) }}</span></div>
                  @endif

                  @if(checkIfWinnerLoserMatch($match['match_number']))
                    <div class="bordered-box"><span class="small">{{ getPlacingWinnerLoserTeam($fixtures, $match, 'away', $groupName, $categoryAge) }}</span></div>
                  @endif
                </div>
              @endforeach
            @endif
