                    ...
                    @if(!in_array($item['page_name'], config('wot.hide_header_menus')))
                        ...
                        <li class="nav-item{{ (isset($item['children']) && count($item['children']) > 0) ? ' dropdown' : '' }}{{ (in_array(Route::currentRouteName(), $item['accessible_routes']) || in_array(Route::currentRouteName(), $childRoutes)) ? ' active' : '' }}">
                            @if(isset($item['children']) && count($item['children']) > 0)
                                <a class="nav-link dropdown-toggle" href="{{  (in_array(Route::currentRouteName(), $item['accessible_routes'])) ? 'javascript:void(0)' : route(config('wot.page_routes')[$item['name']], ['domain' => $websiteDetail->domain_name]) }}" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{!! __('messages.' . $item['name']) !!}</a>
                                <div class="dropdown-menu">
                                    <ul>
                                        <li class="nav-value d-none d-lg-block">
                                            <a href="{{  (in_array(Route::currentRouteName(), $item['accessible_routes'])) ? 'javascript:void(0)' :route(config('wot.page_routes')[$item['name']], ['domain' => $websiteDetail->domain_name]) }}" class="current-tab">{!! __('messages.' . $item['name']) !!}</a>
                                        </li>
                                        @foreach($item['children'] as $childItem)
                                            <li>
                                                ...
                                                @else
                                                    <a href="{{ in_array(Route::currentRouteName(), $childItem['accessible_routes']) ? 'javascript:void(0)' : route(config('wot.page_routes')[$childItem['name']], ['domain' => $websiteDetail->domain_name]) }}" class="{{ in_array(Route::currentRouteName(), $childItem['accessible_routes']) ? 'active' : '' }}">{!! __('messages.' . $childItem['name']) !!}</a>
                                                @endif
                                            </li>
                                        @endforeach
                                    </ul>
                            ...
                            @else
                                <a class="nav-link" href="{{ (in_array(Route::currentRouteName(), $item['accessible_routes']) || in_array(Route::currentRouteName(), $childRoutes)) ? 'javascript:void(0)' : route(config('wot.page_routes')[$item['name']], ['domain' => $websiteDetail->domain_name]) }}">{!! __('messages.' . $item['name']) !!}</a>
                            @endif
                        </li>
                    @endif
                @endforeach
