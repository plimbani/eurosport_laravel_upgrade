    ...
    */
    ...
    public function createCompetationFomat($data)
    {
        ...
        if (! ($data['tournament_format'] === 'basic' && $data['competition_type'] === 'league')) {
            ...
            [$totalTime, $totalmatch, $dispFormatname] = $this->calculateTime($data['tournamentTemplate']['json_data'], $data);
        } elseif ($data['tournament_format'] == 'basic') {
            [$totalTime, $totalmatch, $dispFormatname] = $this->calculateTime($data['tournamentTemplate']['json_data'], $data);
        }
        ...
        if (isset($data['competation_format_id']) && $data['competation_format_id'] != 0) {
            ...
            if ($data['tournament_template_id'] != $data['tournamentTemplate']['id'] || $tournamentTemplateObj->tournament_format != $data['tournament_format'] || ($data['tournament_format'] === 'basic' && ($tournamentTemplateObj->competition_type != $data['competition_type'] || $tournamentTemplateObj->total_teams != $data['total_teams']))) {
                ...
                $matchData = ['tournamentId' => $tournamentId, 'ageGroupId' => $ageGroupId];
    ...
    private function calculateTime($json_data, $data)
    {
        ...
        $roundScheduleData = implode(' - ', $json_data->round_schedule);
        ...
        return [$total_time, $total_matches, $disp_format_name];
    }
    ...
    public function GetCompetationFormat($data)
    {
        ...
        if ($isMobileUsers != '') {
            ...
            $tournament_arr = ['tournament_id' => $data['tournament_id']];
    ...
    public function setTemplateMatchesForSecondRound($group1, $group2, $currentRound)
    {
        ...
        for ($i = 0; $i < count($group1); $i++) {
            ...
            $matchNumberGroup1 = strpos($matchNumberGroup1, '#') > 0 ? str_replace(['th', 'st', 'nd', 'rd'], ['', '', '', ''], $matchNumberGroup1) : $matchNumberGroup1;
            $matchNumberGroup2 = strpos($matchNumberGroup2, '#') > 0 ? str_replace(['th', 'st', 'nd', 'rd'], ['', '', '', ''], $matchNumberGroup2) : $matchNumberGroup2;
    ...
    public function generateRoundFixturesBaseOnTeam($teamSize)
    {
        ...
        $odd = array_values(array_filter($teams, function ($k) {
            return $k % 2 == 1;
        }));
        $even = array_values(array_filter($teams, function ($k) {
            return $k % 2 == 0;
        }));
    ...
    public function generateMatchSchedulePrint($templateData)
    {
        ...
        } elseif ($tournamentFormat === 'basic' && $competitionType === 'league') {
