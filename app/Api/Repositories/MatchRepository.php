...
class MatchRepository
{
    ...
    public function setFlagFixture($data = '')
    {
        ...
        if ($teamData['home_team'] != 0 && $teamData['away_team'] != 0) {
            $teamId = true;
            $teams = [$teamData['home_team'], $teamData['away_team']];
        } else {
            $teamId = false;
            $teams = [$teamData['home_team_placeholder_name'], $teamData['away_team_placeholder_name']];
        }
        ...
        $matchResultCount = TempFixture::where('tournament_id', $teamData['tournament_id'])
            ...
            ->where(function ($query) use ($team_interval, $data) {
    ...
    public function getStanding($tournamentData)
    {
        ...
        if ($head_to_head == true && $competition->is_manual_override_standing == 0) {
            ...
            [$competitionStandings, $sort_head_to_head] = $this->sortByHeadtoHead($competitionStandings, $check_head_to_head_with_key, $tournamentData['tournamentId'], $tournamentData['competitionId'], $tournamentCompetationTemplatesRecord, $remain_head_to_head_with_key);
    ...
    public function sortByHeadtoHead($standingData, $keyConflict, $tournamentId, $compId, $tournamentCompetationTemplatesRecord, $remain_head_to_head_with_key)
    {
        ...
        if (! empty($virtual_lt_sorted)) {
            ...
            return [$standingArray, '1'];
        } else {
            return [$standingArray, '0'];
        }
    ...
    public function headToHeadVirtualLeagueTable($conflictedTeamids, $cta, $ctv, $tournamentId, $compId, $tournamentCompetationTemplatesRecord, $remain_head_to_head_with_key, $standingData)
    {
        ...
        $points_array = $goal_difference = $goal_for = $teamName = [];
        ...
        $conflict_still_in_internal_leaguetable_array = [];
        $remainingSorting = [];

        foreach ($sort_virtual_leaguetable as $skey => $svalue) {
            if (count($svalue) > 1 && ! empty($remain_head_to_head_with_key)) {
    ...
    public function getDrawTable($tournamentData)
    {
        ...
        } elseif (count($team_placeholder_name_arr_all) == 0) {
    ...
    public function setMatchSchedule($data, $allowSchedulingForcefully = false)
    {
        ...
        if ($data['isMultiSchedule'] === false) {
            ...
            $matchData = ['teams' => $teams, 'tournamentId' => $matchData['tournamentId'], 'ageGroupId' => $teamData['age_group_id'], 'teamId' => $teamId];
    ...
    public function matchUnschedule($matchId, $isUnscheduleAll = false)
    {
        ...
        if ($matchData->home_team != 0 && $matchData->away_team != 0) {
            $teamId = true;
            $teamsList = [$matchData->home_team, $matchData->away_team];
        } else {
            $teamId = false;
            $teamsList = [$matchData->home_team_placeholder_name, $matchData->away_team_placeholder_name];
        }
        $tournamentId = $matchData->tournament_id;
        $ageGroupId = $matchData->age_group_id;

        $matchData = ['teams' => $teamsList, 'tournamentId' => $tournamentId, 'ageGroupId' => $ageGroupId, 'teamId' => $teamId];
    ...
    public function matchUnscheduledFixtures($matchData)
    {
        ...
        if (count($conflictedFixtureMatchNumber) > 0) {
    ...
    public function unscheduleFixturesByAgeCategory($matchData)
    {
        ...
        foreach ($ageCategories as $ageCategoryId) {
            $date = ['tournamentId' => $tournamentId, 'ageGroupId' => $ageCategoryId];
    ...
    public function setAutomaticMatchSchedule($data, $allowSchedulingForcefully = false)
    {
        ...
        if ($homeMaximumIntervalMatchResultCount->count() > 0) {
            ...
            $homeMaximumIntervalMatchResultCount = $homeMaximumIntervalMatchResultCount->filter(function ($item) use ($maxsdStartTime, $maxsdEndTime, $maxedStartTime, $maxedEndTime, $teamId, $teamData) {
                $homeTeamCheck = false;
                if ($teamId) {
                    $homeTeamCheck = ($item['home_team'] === $teamData['home_team'] || $item['away_team'] === $teamData['home_team']);
                } else {
        ...
        if ($awayMaximumIntervalMatchResultCount->count() > 0) {
            ...
            $maxsdStartTime = Carbon::createFromFormat('Y-m-d H:i:s', $data['matchStartDate'])->subMinutes($maximum_team_interval);
            $maxsdEndTime = Carbon::createFromFormat('Y-m-d H:i:s', $data['matchStartDate'])->subMinutes(0);
            $awayMaximumIntervalMatchResultCount = $awayMaximumIntervalMatchResultCount->filter(function ($item) use ($maxsdStartTime, $maxsdEndTime, $maxedStartTime, $maxedEndTime, $teamId, $teamData) {
                $awayTeamCheck = false;
                if ($teamId) {
                    $awayTeamCheck = ($item['home_team'] === $teamData['away_team'] || $item['away_team'] === $teamData['away_team']);
                } else {
        ...
        $matchData = ['teams' => $teams, 'tournamentId' => $data['tournamentId'], 'ageGroupId' => $teamData['age_group_id'], 'teamId' => $teamId];
        $matchresult = $this->checkTeamIntervalforMatches($matchData);
